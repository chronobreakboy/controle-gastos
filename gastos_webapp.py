...
def calcular_primeira_fatura(data_compra, fechamento, vencimento):
    ...
    dia = data_compra.day
    fechamento_data = datetime(ano, mes, fechamento)
    if data_compra <= fechamento_data:
        vencimento_data = datetime(ano, mes, vencimento)
        if vencimento_data < data_compra:
            mes += 1
            if mes > 12:
                mes = 1
                ano += 1
...
def carregar_tudo():
    ...
    if not df.empty:
        ...
        df["isHoje"] = df["DataObj"].dt.date == hoje
        df = df.sort_values(by=["isHoje", "DataObj"], ascending=[False, False]).reset_index(drop=True)
    return df
